...
register("nquads", Parser, "rdflib_nquads", "NQuadsParser")
...
def parseNQuadURI(uri):
    import urllib
    parser = NQuadParser()
    u = urllib.urlopen(uri)
    sink = parser.parse(u)
    u.close()
    # for quad in sink.__store.items:
    #    print quad
    print 'Number of NQuads found:', sink.length

def main():
    import sys
    if len(sys.argv) == 2:
        parseNQuadURI(sys.argv[1])
    else: print __doc__
